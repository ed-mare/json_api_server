module SimpleJsonApi # :nodoc:
  ...
  class Builder
    ...
    #  SimpleJsonApi::Fields instance if #add_fields was called. nil otherwise.
    attr_reader :fields

    # SimpleJsonApi::Pagination instance if #add_pagination was called. nil otherwise.
    attr_reader :pagination

    # SimpleJsonApi::Filter instance if #add_filter was called. nil otherwise.
    attr_reader :filter

    # SimpleJsonApi::Include instance if #add_include was called. nil otherwise.
    attr_reader :include

    # SimpleJsonApi::Sort instance if #add_sort was called. nil otherwise.
    attr_reader :sort
    ...
    # Creates SimpleJsonApi::Pagination instance based on request, initial query
    # model and pagination options. Instance is available through
    # the #pagination attribute. For collections only.
    #
    # - <tt>options</tt> - SimpleJsonApi::Pagination options.
    def add_pagination(**options)
      @pagination = SimpleJsonApi::Pagination.new(request, model, options)
      self
    end

    # Creates SimpleJsonApi::Filter instance based on request, initial query
    # model and filter configs. Instance is available through
    # the #filter attribute.
    #
    # - <tt>permitted</tt> - SimpleJsonApi::Filter configs.
    def add_filter(permitted = [])
      @filter = SimpleJsonApi::Filter.new(request, model, permitted)
      self
    end

    # Creates SimpleJsonApi::Include instance based on request, initial query
    # model and include configs. Instance is available through
    # the #include attribute.
    #
    # - <tt>permitted</tt> - SimpleJsonApi::Include configs.
    def add_include(permitted = [])
      @include = SimpleJsonApi::Include.new(request, model, permitted)
      self
    end

    # Creates SimpleJsonApi::Sort instance based on request, initial query
    # model and sort options. Instance is available through
    # the #sort attribute.
    #
    # - <tt>options</tt> - SimpleJsonApi::Sort options.
    def add_sort(**options)
      @sort = SimpleJsonApi::Sort.new(request, model, options)
      self
    end

    # Creates SimpleJsonApi::Fields instance based on request. Instance is
    # available through the #fields attribute.
    def add_fields
      @fields = SimpleJsonApi::Fields.new(request)
      self
    end

    # SimpleJsonApi::Paginator instance for collection if #add_pagination
