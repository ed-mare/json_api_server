...
describe JsonApiServer::Paginator do
  let(:current_page) { 3 }
  let(:total_pages) { 24 }
  let(:per_page) { 10 }
  ...
  describe '#next' do
    ...
    it 'is nil when current_page is last page' do
      p = JsonApiServer::Paginator.new(total_pages, total_pages, per_page, base_url, **params)
      expect(p.next).to eq(nil)
    end

    it 'is nil when current_page is invalid' do
      p = JsonApiServer::Paginator.new(-2, total_pages, per_page, base_url, **params)
      expect(p.next).to eq(nil)

      p1 = JsonApiServer::Paginator.new(total_pages + 1, total_pages, per_page, base_url, **params)
  ...
  describe '#prev' do
    ...
    it 'is nil when current_page is first page' do
      p = JsonApiServer::Paginator.new(1, total_pages, per_page, base_url, **params)
    ...
    it 'is nil when current_page is invalid' do
      ...
      p1 = JsonApiServer::Paginator.new(total_pages + 2, total_pages, per_page, base_url, **params)
