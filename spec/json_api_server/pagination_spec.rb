...
describe JsonApiServer::Pagination do
  let(:max_per_page) { 5 }
  ...
  let(:pagination_with_opts) do
    JsonApiServer::Pagination.new(request_all, Topic, max_per_page: max_per_page,
                                                      default_per_page: default_per_page)
  end
  let(:pagination_no_opts) do
    JsonApiServer::Pagination.new(request_all, Topic)
  end
  let(:pagination_bad_params) do
    JsonApiServer::Pagination.new(request_bad_values, Topic, max_per_page: max_per_page,
                                                             default_per_page: default_per_page)
  end
  let(:pagination_negative_params) do
    JsonApiServer::Pagination.new(request_negative_values, Topic, max_per_page: max_per_page,
                                                                  default_per_page: default_per_page)
  end
  let(:pagination_no_params) do
    JsonApiServer::Pagination.new(request_none, Topic, max_per_page: max_per_page,
                                                       default_per_page: default_per_page)
  end
  ...
  describe '#initialize' do
    ...
    it 'defaults max_per_page to JsonApiServer.configuration.default_max_per_page' do
      expect(pagination_no_opts.max_per_page).to eq(JsonApiServer.configuration.default_max_per_page)
    end

    it 'defaults default_per_page to JsonApiServer.configuration.default_per_page' do
      expect(pagination_no_opts.default_per_page).to eq(JsonApiServer.configuration.default_per_page)
    end
  end

  describe '#number' do
    it 'is an alias to #page' do
      expect(JsonApiServer::Pagination.instance_method(:number)).to eq(
        JsonApiServer::Pagination.instance_method(:page)
      )
    end
  end
  ...
  describe '#limit' do
    it 'is an alias to #per_page' do
      expect(JsonApiServer::Pagination.instance_method(:limit)).to eq(
        JsonApiServer::Pagination.instance_method(:per_page)
      )
    end
  end
  ...
  describe '#per_page' do
    ...
    it 'is :max_per_page when limit > max_per_page' do
      ...
      pagination = JsonApiServer::Pagination.new(request, Topic, max_per_page: 6)
  ...
  describe '#paginator_for' do
    ...
    let(:pagination) { JsonApiServer::Pagination.new(request_all, Topic, max_per_page: 10) }
    let(:paginator) { pagination.paginator_for(pagination.query) }

    it 'returns a JsonApiServer::Paginator object' do
      expect(paginator).to be_instance_of(JsonApiServer::Paginator)
    end

    it 'sets the paginator attributes with values from the Pagination instance' do
      base_url = 'http://localhost:3001/fake' #  JsonApiServer.configuration.base_url + request.path
  ...
  describe '#query' do
    it 'is an alias to #relation' do
      expect(JsonApiServer::Pagination.instance_method(:query)).to eq(
        JsonApiServer::Pagination.instance_method(:relation)
      )
    end
  end
