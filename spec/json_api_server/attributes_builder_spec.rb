...
describe JsonApiServer::AttributesBuilder do
  ...
  describe '#fields' do
    it 'is assigned in initialize' do
      b = JsonApiServer::AttributesBuilder.new(fields)
      expect(b.fields).to eq(fields)
    end

    it 'stringifies array elements' do
      a = fields.map(&:to_sym)
      b = JsonApiServer::AttributesBuilder.new(a)
      expect(b.fields).to eq(fields)
    end

    it 'defaults to nil' do
      b = JsonApiServer::AttributesBuilder.new
  ...
  describe 'add' do
    it 'is chainable' do
      b = JsonApiServer::AttributesBuilder.new(fields1)
      expect(b.add('title', 'the title')).to be(b)
    end

    it 'adds when name is in fields array' do
      attrs = JsonApiServer::AttributesBuilder.new(fields1)
                                              .add('title', 'the title')
                                              .add('body', 'the body')
    ...
    it 'adds all if fields array is nil' do
      attrs = JsonApiServer::AttributesBuilder.new
                                              .add('title', 'the title')
                                              .add('body', 'the body')
                                              .add('author_name', 'the name')
  ...
  describe 'add_if' do
    it 'adds when the proc returns true' do
      attrs = JsonApiServer::AttributesBuilder.new(fields)
