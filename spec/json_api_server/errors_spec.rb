...
describe JsonApiServer::Errors do
  include_context 'errors shared context'

  describe '#initialize' do
    it 'accepts an error hash' do
      e = JsonApiServer::Errors.new(error_1_hash)
    ...
    it 'accepts nil' do
      e = JsonApiServer::Errors.new(nil)
      expect(e.as_json).to eq(nil_as_json)
      e1 = JsonApiServer::Errors.new([nil, nil])
  ...
  describe '#as_json' do
    ...
    it 'serializes partial error attributes' do
      e = JsonApiServer::Errors.new(error_2_hash)
      expect(e.as_json).to eq(error_2_as_json)
    end

    it 'serializes multiple errors' do
      e = JsonApiServer::Errors.new([error_1_hash, error_2_hash])
