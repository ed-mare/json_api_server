...
describe JsonApiServer::Filter do
  let(:configs) do
    [
      { id: { type: 'Integer' } },
      { published: { type: 'Date' } },
  ...
  describe '#initialize' do
    let(:filter) { JsonApiServer::Filter.new(request_by_id, Topic, configs) }
    let(:filter_no_params) { JsonApiServer::Filter.new(request_empty, Topic, configs) }
  ...
  describe '#meta_info' do
    ...
    it 'describes filters (for meta element)' do
      filter = JsonApiServer::Filter.new(request, Topic, configs)
    ...
    it 'removes filters with blank values' do
      ...
      filter = JsonApiServer::Filter.new(req, Topic, configs)
  ...
  describe '#query' do
    it 'is an alias to #relation' do
      expect(JsonApiServer::Filter.instance_method(:query)).to eq(
        JsonApiServer::Filter.instance_method(:relation)
      )
    end
  end
  ...
  describe '#relation' do
    describe 'casts values to specified type in filter configs' do
      ...
      it '- ex: converts to an integer' do
        filter = JsonApiServer::Filter.new(request_num, Topic, configs)
        expect(filter.relation.to_sql).to match(/= 1/)
      end

      it '- ex: converts to a string' do
        filter = JsonApiServer::Filter.new(request_string, Topic, configs)
        expect(filter.relation.to_sql).to match(/= '1'/)
      end

      it '- ex: converts to a Date' do
        filter = JsonApiServer::Filter.new(request_date, Topic, configs)
        expect(filter.relation.to_sql).to match(/'2017-09-04'/)
      end

      it '- ex: converts unrecongizable Date to nil' do
        filter = JsonApiServer::Filter.new(request_bad_date, Topic, configs)
        expect(filter.relation.to_sql).to match(/\"topics\".\"published\" = NULL/)
      end

      it '- ex: converts to a DateTime (calls :in_time_zone)' do
        filter = JsonApiServer::Filter.new(request_datetime, Topic, configs)
      ...
      it '- ex: converts unrecongizable DateTime to nil' do
        filter = JsonApiServer::Filter.new(request_bad_datetime, Topic, configs)
    ...
    describe 'wildcards' do
      ...
      it 'the left when configured for :left and param begins with *' do
        filter = JsonApiServer::Filter.new(request, Topic, config_left)
      ...
      it 'the right when configured for :right and param begins with *' do
        filter = JsonApiServer::Filter.new(request, Topic, config_right)
        expect(filter.relation.to_sql).to match(/\("topics"."book" LIKE 'Harry Potter%'\)/)
      end

      it 'both sides when configured for :both and param begins with *' do
        filter = JsonApiServer::Filter.new(request, Topic, config_both)
        expect(filter.relation.to_sql).to match(/\("topics"."book" LIKE '%Harry Potter%'\)/)
      end

      it 'only when param begin with *' do
        filter = JsonApiServer::Filter.new(request_wo_wildcard, Topic, config_both)
    ...
    describe 'accepts operators' do
      ...
      it '> and >=' do
        filter_gt = JsonApiServer::Filter.new(request_gt, Topic, configs)
        filter_gte = JsonApiServer::Filter.new(request_gte, Topic, configs)
        expect(filter_gt.relation.to_sql).to match(/"topics"."id" > 1/)
        expect(filter_gte.relation.to_sql).to match(/"topics"."id" >= 1/)
      end

      it '< and <=' do
        filter_lt = JsonApiServer::Filter.new(request_lt, Topic, configs)
        filter_lte = JsonApiServer::Filter.new(request_lte, Topic, configs)
      ...
      it '= and !=' do
        filter_eq = JsonApiServer::Filter.new(request_eq, Topic, configs)
        filter_neq = JsonApiServer::Filter.new(request_neq, Topic, configs)
      ...
      it 'which can be combined' do
        filter = JsonApiServer::Filter.new(request_between, Topic, configs)
        re = Regexp.escape(%[("topics"."published" > '1998-01-01') AND ("topics"."published" < '1999-12-31')])
        expect(filter.relation.to_sql).to match(/#{re}/)
      end

      it 'and defaults to = if none of the above' do
        filter = JsonApiServer::Filter.new(request_op_unknown, Topic, configs)
    ...
    describe 'with builder: :model_query configs' do
      ...
      let(:filter) { JsonApiServer::Filter.new(request, Topic, configs) }
    ...
    describe 'with custom builder configs' do
      # custom builder in test_app/config/initializers/json_api_server.rb
      let(:configs) do
        [
    ...
    it 'raises a JsonApiServer::BadRequest if an unpermitted attribute is requested' do
      filter = JsonApiServer::Filter.new(request_invalid_attr, Topic, configs)
      expect { filter.relation }.to raise_exception(JsonApiServer::BadRequest,
