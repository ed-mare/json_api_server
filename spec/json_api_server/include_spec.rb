...
describe JsonApiServer::Include do
  ...
  describe '#initialize' do
    it 'assigns request attr' do
      i = JsonApiServer::Include.new(request_empty, Topic)
    ...
    it 'assigns params attr to request.query_parameters' do
      i = JsonApiServer::Include.new(request_1, Topic)
    ...
    it 'assigns permitted to empty hash if assigned a non-array' do
      i = JsonApiServer::Include.new(request_empty, Topic, 'foobar')
      expect(i.permitted).to eq([])
    end
  end
  ...
  describe '#include_params' do
    it 'converts to array' do
      ...
      i = JsonApiServer::Include.new(request_0, Topic)
      expect(i.include_params).to eq([])

      i = JsonApiServer::Include.new(request_1, Topic)
      expect(i.include_params).to eq(['publisher'])

      i = JsonApiServer::Include.new(request_3, Topic)
  ...
  describe '#includes' do
    it 'raises an exception when invalid includes are specified (#relation called)' do
      i = JsonApiServer::Include.new(
        request_1,
        Topic,
        ['comments' => Topic.includes(:comments)]
      )
      expect { i.includes }.to raise_error(JsonApiServer::BadRequest,
  ...
  describe '#query' do
    it 'is an alias to #relation' do
      expect(JsonApiServer::Include.instance_method(:query)).to eq(
        JsonApiServer::Include.instance_method(:relation)
      )
    end
  end
  ...
  describe '#relation' do
    ...
    it 'raises exception if requested include is not permitted' do
      ...
      expect { i.relation }.to raise_error(JsonApiServer::BadRequest,
                                           /Inclusion param 'publisher' is not supported/)
    end
