...
describe SimpleJsonApi::ValidationErrors do
  let(:model) do
    class MyModel
  ...
  describe '#as_json' do
    it 'returns one validation error per attribute' do
      e = SimpleJsonApi::ValidationErrors.new(model)
    ...
    it 'returns no errors (empty array) when no validation errors' do
      ...
      e = SimpleJsonApi::ValidationErrors.new(sam)
      expect(e.as_json).to eq(
        'jsonapi' => { 'version' => '1.0' },
        'errors' =>  []
      )
    ...
    it 'returns no errors (empty array) when object is not model' do
      e = SimpleJsonApi::ValidationErrors.new(nil)
      expect(e.as_json).to eq(
        'jsonapi' => { 'version' => '1.0' },
        'errors' => []
      )
