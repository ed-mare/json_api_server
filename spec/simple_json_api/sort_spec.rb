...
describe SimpleJsonApi::Sort do
  ...
  let(:sort_empty) { SimpleJsonApi::Sort.new(request_empty, Topic, options_multi) }
  let(:sort_one) { SimpleJsonApi::Sort.new(request_one, Topic, options_multi) }
  let(:sort_multi) { SimpleJsonApi::Sort.new(request_multi, Topic, options_multi) }
  let(:sort_none) { SimpleJsonApi::Sort.new(request_no_sort, Topic, options_multi) }
  let(:sort_no_default) { SimpleJsonApi::Sort.new(request_no_sort, Topic, options_no_default) }
  let(:sort_no_opts) { SimpleJsonApi::Sort.new(request_no_sort, Topic, {}) }
  let(:sort_column_alias) { SimpleJsonApi::Sort.new(request_column_alias, Topic, options_multi) }
  ...
  describe '#configs' do
    it 'is an instance of SimpleJsonApi::SortConfigs' do
      expect(sort_one.configs).to be_an_instance_of(SimpleJsonApi::SortConfigs)
    end
  end

  describe '#query' do
    it 'is an alias to #relation' do
      expect(SimpleJsonApi::Sort.instance_method(:query)).to eq(
        SimpleJsonApi::Sort.instance_method(:relation)
      )
    end
  end
  ...
  describe '#relation' do
    it 'raises an exception when requested sort attributes are not in the permitted list' do
      sort = SimpleJsonApi::Sort.new(request_no_match, Topic, options_multi)
      expect { sort.relation }.to raise_error(SimpleJsonApi::BadRequest,
