...
describe SimpleJsonApi::ResourcesSerializer do
  ...
  let(:filter) { SimpleJsonApi::Filter.new(request, Topic, %i[character location published]) }
  let(:paginator) { SimpleJsonApi::Paginator.new(2, 20, 10, 'http://foo') }

  describe '#initialize' do
    it 'sets objects instance variable to objects if serializer is not set' do
      s = SimpleJsonApi::ResourcesSerializer.new(objects)
      expect(SimpleJsonApi::ResourcesSerializer.objects_serializer).to eq(nil)
      expect(s.instance_variable_get(:@objects)).to be(objects)
    end

    it 'sets filter option' do
      s = SimpleJsonApi::ResourcesSerializer.new(objects, filter: filter)
      expect(s.filter).to be(filter)
    end
    it 'sets paginator option' do
      s = SimpleJsonApi::ResourcesSerializer.new(objects, paginator: paginator)
      expect(s.paginator).to be(paginator)
    end
    it 'sets include option' do
      s = SimpleJsonApi::ResourcesSerializer.new(objects, includes: includes)
      expect(s.includes).to be(includes)
    end
    it 'sets fields option' do
      s = SimpleJsonApi::ResourcesSerializer.new(objects, fields: fields)
      expect(s.fields).to be(fields)
    end
    it 'sets as_json_options option' do
      s = SimpleJsonApi::ResourcesSerializer.new(objects, as_json_options: { include: [:data] })
  ...
  describe '.from_builder' do
    let(:builder) do
      SimpleJsonApi::Builder.new(request, Topic.all)
    ...
    it 'returns an instance of ResourceSerializer with options set' do
      s = SimpleJsonApi::ResourcesSerializer.from_builder(builder)
      expect(s).to be_an_instance_of(SimpleJsonApi::ResourcesSerializer)
