...
describe SimpleJsonApi::BaseSerializer do
  class MyClass < SimpleJsonApi::BaseSerializer
    def as_json
      {
  ...
  class ExampleSerializer < SimpleJsonApi::BaseSerializer
    def links
      {
  ...
  let(:base) { SimpleJsonApi::BaseSerializer.new }
  ...
  describe '#as_json' do
    ...
    it 'overrides options set in as_json_options' do
      base1 = SimpleJsonApi::BaseSerializer.new
  ...
  describe '#base_url (protected)' do
    it 'points to Configuration#base_url' do
      expect(base.send(:base_url)).to eq(SimpleJsonApi.configuration.base_url)
    end
  end
end
